...
import { Currency, CurrencyAmount, Fraction, JSBI, Percent, Token, TokenAmount, WTRX } from '@oikos/swap-sdk'
      ...
      */}
      ...
      <RowBetween my="1rem">
        <Text fontSize={16} fontWeight={500}>
          Pooled {chainId && token.equals(WTRX[chainId]) ? 'WTRX' : token.symbol}:
        </Text>
        <RowFixed>
      ...
      <RowBetween mb="1rem">
        ...
        <RowFixed>
          ...
          <CurrencyLogo size="20px" style={{ marginLeft: '8px' }} currency={Currency.TRON} />
        </RowFixed>
      </RowBetween>
    </>
  )
...
function V1PairMigration({ liquidityTokenAmount, token }: { liquidityTokenAmount: TokenAmount; token: Token }) {
  ...
  const [v2PairState, v2Pair] = usePair(chainId ? WTRX[chainId] : undefined, token)
  const isFirstLiquidityProvider: boolean = v2PairState === PairState.NOT_EXISTS

  const v2SpotPrice = chainId && v2Pair ? v2Pair.reserveOf(token).divide(v2Pair.reserveOf(WTRX[chainId])) : undefined
  ...
  return (
    <BodyWrapper style={{ padding: 24 }}>
      <AutoColumn gap="16px">
        ...
        ) : validatedAddress && chainId && token?.equals(WTRX[chainId]) ? (
          <>
            <TYPE.body my={9} style={{ fontWeight: 400 }}>
