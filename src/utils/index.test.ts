...
import { TokenAmount, Token, ChainId, Percent, JSBI } from '@oikos/swap-sdk'
...
describe('utils', () => {
  describe('#getEtherscanLink', () => {
    ...
    it('correct for token', () => {
      expect(getEtherscanLink(1, 'abc', 'token')).toEqual('https://tronscan.io/token/abc')
    })
    it('correct for address', () => {
      expect(getEtherscanLink(1, 'abc', 'address')).toEqual('https://tronscan.io/address/abc')
    })
    it('unrecognized chain id defaults to mainnet', () => {
      expect(getEtherscanLink(2, 'abc', 'address')).toEqual('https://tronscan.io/address/abc')
    })
    it('ropsten', () => {
      expect(getEtherscanLink(3, 'abc', 'address')).toEqual('https://ropsten.tronscan.io/address/abc')
    })
    it('enum', () => {
      expect(getEtherscanLink(ChainId.NILE, 'abc', 'address')).toEqual('https://rinkeby.tronscan.io/address/abc')
    })
  })
