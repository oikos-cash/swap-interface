import { CurrencyAmount, TRON, Percent, Route, TokenAmount, Trade } from '@oikos/swap-sdk'
...
describe('v1SwapArguments', () => {
  const USDC_WTRX = new MockV1Pair('1000000', new TokenAmount(USDC, '1000000'))
  const DAI_WTRX = new MockV1Pair('1000000', new TokenAmount(DAI, '1000000'))

  // just some random address
  const TEST_RECIPIENT_ADDRESS = USDC_WTRX.liquidityToken.address
  ...
  it('exact eth to token', () => {
    const trade = Trade.exactIn(new Route([USDC_WTRX], TRON), CurrencyAmount.tron('100'))
  ...
  it('exact token to eth', () => {
    const trade = Trade.exactIn(new Route([USDC_WTRX], USDC, TRON), new TokenAmount(USDC, '100'))
    const result = v1SwapArguments(trade, {
      recipient: TEST_RECIPIENT_ADDRESS,
      allowedSlippage: new Percent('1', '100'),
      ttl: 20 * 60
  ...
  it('exact token to token', () => {
    const trade = Trade.exactIn(new Route([USDC_WTRX, DAI_WTRX], USDC), new TokenAmount(USDC, '100'))
    const result = v1SwapArguments(trade, {
      recipient: TEST_RECIPIENT_ADDRESS,
      allowedSlippage: new Percent('1', '100'),
      ttl: 20 * 60
  ...
  it('eth to exact token', () => {
    const trade = Trade.exactOut(new Route([USDC_WTRX], TRON), new TokenAmount(USDC, '100'))
    const result = v1SwapArguments(trade, {
      recipient: TEST_RECIPIENT_ADDRESS,
      allowedSlippage: new Percent('1', '100'),
      ttl: 20 * 60
  ...
  it('token to exact eth', () => {
    const trade = Trade.exactOut(new Route([USDC_WTRX], USDC, TRON), CurrencyAmount.tron('100'))
    const result = v1SwapArguments(trade, {
      recipient: TEST_RECIPIENT_ADDRESS,
      allowedSlippage: new Percent('1', '100'),
      ttl: 20 * 60
  ...
  it('token to exact token', () => {
    const trade = Trade.exactOut(new Route([USDC_WTRX, DAI_WTRX], USDC), new TokenAmount(DAI, '100'))
    const result = v1SwapArguments(trade, {
      recipient: TEST_RECIPIENT_ADDRESS,
      allowedSlippage: new Percent('1', '100'),
      ttl: 20 * 60
